...
import { type OrderProp, changeOrderState } from "@/redux/order/orderSlice"
...
export function OrderComponent({
  ...
  orders: OrderProp[]
  states: OrderStateProp[]
}) {
  const user = useSelector((state: RootState) => state.user)

  const [open, setOpen] = useState<boolean>(false)
  const [selectedOrder, setSelectedOrder] = useState<OrderProp | null>(null)
  const [selectedState, setSelectedState] = useState<OrderStateProp | null>(
    null
  )

  const dispatch = useDispatch<AppDispatch>()

  const handleClick = (order: OrderProp, state: OrderStateProp) => {
